...
const config = {
    A187: '👾A062BOT👾',
    instagram: 'https://instagram.com/andhikavanz',
    nomer: 'wa.me/6285712629044',
    aktif: '24 JAM',
    youtube: 'https://youtube.com/channel/UCp4v6T4jk2ikyc9OTAnAeJA',
    whatsapp: 'https://chat.whatsapp.com/JVl8dTrp9MMKuDmdLqwiT0',
    tanggal: `TANGGAL: ${moment().format('DD')} ${bulan} ${moment().format('YYYY')}`,
    waktu: time
}

const { A062, tanggal, waktu, instagram, whatsapp, youtube, nomer, aktif, ontime } = config
...
client.on('qr', qr => {
   ...
   console.log(`[ ${time} ] QR code is ready, subscribe AndhikaVan`)
})
...
client.on('message-status-update', json => {
   ...
   console.log(`[ ${time} ] => bot by ig:@andhikavanz`)
})
...
client.on('message-new', async (m) => {
   ...
   switch (prefix) {
       case 'help':
           client.sendMessage(id, help.help(id, A062, tanggal, whatsapp, youtube, instagram, nomer),MessageType.text)
           break      
      case 'donate':
           client.sendMessage(id, donate.donate(id, A062, tanggal),MessageType.text)
           break          
      case 'info':
           client.sendMessage(id, info.info(id, A062, tanggal),MessageType.text)
           break             
       case 'nulis':
           nulis(value)
               .then(data => {
